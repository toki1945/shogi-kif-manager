...
export function newGame(kif: string, filename?: string, title?: string): Game {
  const { headers } = parseKif(kif);
  return { id: crypto.randomUUID(), title: title?.trim() || filename?.replace(/\.(kif|kifu)$/i, "") || headers["棋戦"] || "無題の棋譜", kif, tags: [], favorite: false, note: "", created_at: new Date().toISOString() };
}
