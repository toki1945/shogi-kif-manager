...
import { ArrowDownToLine, ArrowUpRight, Bookmark, BookOpen, Check, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, CircleHelp, Cloud, FileText, FolderOpen, LayoutGrid, LoaderCircle, LogOut, Pause, Play, Plus, RotateCw, Search, Tags, Trash2, Upload, Volume2, VolumeX, X } from "lucide-react";
...
export default function KifuApp() {
  ...
  const playMoveSound = useCallback((force = false) => {
    ...
    try {
      ...
      high.frequency.exponentialRampToValueAtTime(240, now + 0.045);
      low.type = "sine";
      low.frequency.setValueAtTime(150, now);
      low.frequency.exponentialRampToValueAtTime(90, now + 0.075);
      gain.gain.setValueAtTime(0.0001, now);
      gain.gain.exponentialRampToValueAtTime(0.2, now + 0.004);
      gain.gain.exponentialRampToValueAtTime(0.0001, now + 0.085);
      high.connect(gain); low.connect(gain); gain.connect(context.destination);
      high.start(now); low.start(now);
      high.stop(now + 0.05); low.stop(now + 0.085);
    } catch {
      // Audio may be unavailable in restricted browser contexts; playback still works silently.
    }
  }, [soundEnabled]);
  ...
  useEffect(() => {
    const saved = localStorage.getItem("kifucho-sound-enabled");
    if (saved !== null) setSoundEnabled(saved === "true");
    return () => { void audioContext.current?.close(); audioContext.current = null; };
  }, []);
  useEffect(() => { previousStep.current = 0; setStep(0); setPlaying(false); }, [selected]);
  ...
  useEffect(() => {
    ...
    if (!parsed || previous === step) return;
    const traversedMove = parsed.moves[Math.max(previous, step) - 1];
    if (traversedMove && !traversedMove.end) playMoveSound();
  }, [step, parsed, playMoveSound]);
  ...
  async function importGames(items: { text: string; filename?: string; title?: string }[]) {
    try {
      const added = items.map(({ text, filename, title }) => newGame(text, filename, title));
      if (!added.length) return;
      if (await persist([...added, ...games], added)) {
        setSelected(added[0].id); setFilter("all"); setQuery(""); setModal(null); setImportText(""); setImportTitle(""); setImportFilename(""); showNotice(`${added.length}件の棋譜を取り込みました。`);
      }
  ...
  async function readFiles(files: FileList | File[]) {
    try {
      ...
      if (!file) return;
      if (files.length > 1) throw new Error("棋譜は1件ずつ選択してください。");
      ...
      const text = decodeKif(await file.arrayBuffer());
      parseKif(text);
      setImportText(text);
      setImportFilename(file.name);
      setImportTitle(file.name.replace(/\.(kif|kifu)$/i, ""));
    } catch (error) { showNotice(messageOf(error), true); }
  ...
  function toggleSound() {
    ...
    localStorage.setItem("kifucho-sound-enabled", String(next));
    if (next) playMoveSound(true);
  }
  ...
  return <div className="app-shell">
    ...
    <dialog ref={dialog} className="modal" onCancel={() => setModal(null)} onClick={e => { if (e.target === e.currentTarget && !busy) setModal(null); }} aria-labelledby="modal-title"><div className="modal-content"><button className="modal-close icon-button" aria-label="閉じる" disabled={busy} onClick={() => setModal(null)}><X size={20} /></button>
      {modal === "import" && <><div className="modal-icon"><Upload size={24} /></div><h2 id="modal-title">新しい一局を、棋譜帖に。</h2><p>KIF ファイルを選ぶか、棋譜を貼り付けて、分かりやすい名称を付けてください。</p><label className="form-label import-title">棋譜名<span>あとから変更できます</span><input type="text" maxLength={200} placeholder="例：県大会 決勝戦" value={importTitle} onChange={e => setImportTitle(e.target.value)} /></label><button className={`drop-zone ${importFilename ? "file-selected" : ""}`} disabled={busy} onClick={() => fileInput.current?.click()} onDragOver={e => e.preventDefault()} onDrop={e => { e.preventDefault(); if (!busy) void readFiles(e.dataTransfer.files); }}><Upload size={28} /><strong>{importFilename || "ファイルを選択、またはドロップ"}</strong><span>{importFilename ? "別のファイルを選ぶ" : ".kif / .kifu · UTF-8 / Shift_JIS · 1件 2 MB まで"}</span></button><input type="file" ref={fileInput} hidden accept=".kif,.kifu" onChange={e => { if (e.target.files) void readFiles(e.target.files); }} /><div className="or-divider">または棋譜を貼り付け</div><textarea className="kif-input" aria-label="KIF テキスト" placeholder={"先手：\n後手：\n手数----指手---------消費時間--\n1 ７六歩(77)\n2 ３四歩(33)"} value={importText} onChange={e => { setImportText(e.target.value); setImportFilename(""); }} /><button className="button primary full-width" disabled={!importText.trim() || busy} onClick={() => void importGames([{ text: importText, filename: importFilename || undefined, title: importTitle }])}>{busy ? <LoaderCircle className="spin" size={17} /> : <Plus size={17} />}「{importTitle.trim() || "名称未設定"}」を取り込む</button></>}
